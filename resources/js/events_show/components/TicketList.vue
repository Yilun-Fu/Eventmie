<template>
    <div>
        ...
        <button class="lgx-btn lgx-btn-success text-center" type="button" @click="openModal = true"><i
                class="fas fa-ticket-alt"></i> {{ trans('em.get') }} {{ trans('em.tickets') }}</button>

        <div class="modal modal-mask" v-if="openModal">
            <div class="modal-dialog modal-container modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" @click="close()"><span
                    ...
                    <form ref="form" @submit.prevent="validateForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row ml-1" v-if="customers.length > 0">
                            <div class="col-md-4">
                                <div class="form-group text-left">
                                    ...
                                    <select class="form-control" name="customer_id" v-model="customer_id"
                                    ...
                                    <li class="list-group-item">
                                        <div class="row">
                                            <div class="col-md-4">
                                                ...
                                                <div class="form-group text-left">
                                                    <label class="text-left">{{ trans('em.select') }} {{
                                                        trans('em.quantity') }}</label>
                                                    <select class="form-control" name="quantity" v-model="quantity">
                                                        <option value="0" selected>0</option>
                                                        <option :value="itm" v-for=" (itm, ind) in max_ticket_qty"
                                                            :key="ind">{{ itm }}</option>
                                                    </select>
                                                </div>
                                            </div>
                                ...
                                <ul class="list-group">
                                    <li class="list-group-item">
                                        <div class="form-group">
                                            <label for="ticket_file">上传文件:</label>
                                            <input type="file" name="ticket_file" class="form-control" ref="ticket_file"
                                                required>
                                        </div>
                                    </li>
                                </ul>
                        ...
                        <div class="row mt-2">
                            <div class="col-xs-12">
                                <button type="button" class="lgx-btn lgx-btn-red btn-block" @click="bookTickets()"><i
                                        class="fas fa-cash-register"></i> {{ trans('em.checkout') }}</button>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        bookTickets() {
            ...
            if (this.$refs.ticket_file && this.$refs.ticket_file.files[0]) {
                post_data.append('ticket_file', this.$refs.ticket_file.files[0]);
            }
            ...
            axios.post(post_url, post_data)
                ...
                .catch(error => {
                    // only in case of serverValidate
                    if (error.response && error.response.data) {
                        this.serverValidate(error.response.data.errors);
                    }
                });
        },
